...
model=os.getenv("MODEL", "gpt-4o-mini")  # Default to gpt-4o-mini if not set
...
def get_completion(dialogs, temperature=0, max_tokens=100):
    ...
    for i in range(max_retries):
        try:
            response = client.chat.completions.create(model=model, # inaccessible now, try gpt-4o-mini
            messages=dialogs,
